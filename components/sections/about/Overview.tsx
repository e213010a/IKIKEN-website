          ...
          <dl className="mt-12 divide-y divide-navy-950/10 border-t border-navy-950/10">
            {rows.map((row) => (
              <div key={row.label} className="grid gap-1 py-5 sm:grid-cols-[160px_1fr] sm:gap-6">
                <dt className="text-sm font-medium tracking-[0.15em] text-ink-muted sm:text-base">{row.label}</dt>
                <dd className="text-sm tracking-[0.15em] text-navy-950 sm:text-base lg:text-lg">{row.value}</dd>
              </div>
            ))}
          </dl>
        </FadeIn>
